...
class Alphabet extends FlxSpriteGroup
{
	...
	//Old habits die hard, I'm adding targetX and forceX from the old build in for sanity lol -Whatify
	...
	public var scaleWord(default, set):Float = 1;
	...
	private function set_scaleWord(value:Float)
		{
			...
			for (letter in letters)
			{
				if(letter != null)
				{
					...
					letter.spawnScale.x = value;
					letter.spawnScale.y = value;
					letter.x = letter.spawnPos.x * ratio + (letter.x-letter.spawnPos.x) ;
				}
			}
			return value;
		}
	...
	override function update(elapsed:Float)
	{
		if (isMenuItem)
		{
			...
			//Legacy menu x settings from VS Nikko builds
