...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		var enterTextColors:Array<FlxColor> = getIntroColors();
		if(enterTextColors.length>0){
			titleTextColors = enterTextColors;
		}
		// DEBUG BULLSHIT
	...
	{
		var swagGoodArray:Array<FlxColor> = [];
		try{
			var fullText:String = Assets.getText(Paths.txt('titleColors'));
		
			var firstArray:Array<String> = fullText.split('\n');
			
			for (i in firstArray)
			{
				var color1:FlxColor = Std.parseInt(i);
				if(!i.startsWith('0x')) color1 = Std.parseInt('0xff' + i);
				swagGoodArray.push(color1);
			}
		}
		catch(err){
			//meh
	...
	{
		while (textGroup.members!=null&&textGroup.members.length > 0)
		{
